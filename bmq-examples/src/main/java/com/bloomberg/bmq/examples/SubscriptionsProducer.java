...
/**
 * Sample application which connects to BlazingMQ broker, takes user input and send messages. Part
 ...
 *
 * <p>In order to receive ack messages, {@code Producer} class implements {@link AckMessageHandler}
 * interface. When new ack messages are being delivered from the broker, {@link #handleAckMessage}
 * method is called.
 *
 * <p>Also, {@code Producer} implements {@link SessionEventHandler} and {@link QueueEventHandler}
 ...
 */
public class SubscriptionsProducer
    ...
    private static String[] firmIds = {"FOO", "BAR", "FOO", "BAR", "BAR", "BAR"};

    private static int[] prices = {197, 23, 186, 29, 28, 24};

    SubscriptionsProducer() {
    ...
    public void post(String payload) {
        ...
        String firmId = firmIds[currentMessageId % firmIds.length];
        int price = prices[currentMessageId % prices.length];

        mp.setPropertyAsString("firmId", firmId);
        mp.setPropertyAsInt32("price", price);
    ...
    public static void main(String[] args) {
        ...
        SubscriptionsProducer producer = new SubscriptionsProducer();
