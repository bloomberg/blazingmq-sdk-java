...
/**
 * Provides access to the BMQ broker.
 *
 ...
 * must be deployed in the BMQ infrastructure prior to opening queues under that domain, because
 * opening a queue actually loads the metadata deployed for the associated domain.
 *
 ...
 *
 * <p>Queue are identified by URIs (Unified Resource Identifiers) that must follow the BMQ syntax,
 * manipulated as {@link com.bloomberg.bmq.Uri} objects. A queue URI is typically formatted as
 * follows:
 *
 * <pre>
 *
 ...
 * </pre>
 *
 * Note that domain names are unique in BMQ infrastructure, which makes a fully qualified queue URI
 * unique too.
 *
 * <p>Queues in BMQ infrastructure are created by applications on demand. Broker creates a queue
 * when it receives an open-queue request from an application for a queue that does not exist
 * currently.
 *
...
public final class Session implements AbstractSession {
    ...
    /**
     * Connects to the BMQ broker and start the message processing.
     *
    ...
    /**
     * Gracefully disconnects from the BMQ broker and stop the operation of this session.
     *
        ...
        if (handler == null) {
            String msg =
                    "Queue event handler was not provided. It will be mandatory in "
                            + "one of the the upcoming releases of BMQ Java SDK. Please provide "
                            + "non-null and valid queue event handler.  Please reach out to BMQ "
