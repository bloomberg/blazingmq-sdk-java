...
public class EventQueueStatsTest {
    ...
    public void testSampleOutput() throws Exception {
        ...
        String afterFinalExpected = sampleAfterFinalOutput();
        String afterFinalActual = builder.toString();

        logger.info("After final expected:\n{}", afterFinalExpected);
        logger.info("After final actual:\n{}", afterFinalActual);

        assertEquals(afterFinalExpected, afterFinalActual);

        // Check final output
        builder = new StringBuilder();
        stats.dump(builder, true);

        finalActual = builder.toString();
    ...
    private String sampleAfterFinalOutput() throws IOException {
        return readFileContent(MessagesTestSamples.STATS_EVENTQUEUE_SAMPLE_AFTER_FINAL.filePath());
    }
